...
export const RecipesTab: React.FC = () => {
  ...
  const handleApply = async (template: EditingTemplate): Promise<void> => {
    if (!selectedClip || !selectedTargetType) {
      toast.warning(
        t('recipes.select_clip'),
        t('recipes.apply_on_one'),
      );
      return;
    }
    ...
    try {
      ...
      if (!applicationId) {
        toast.error(
          t('recipes.err_apply'),
          t('recipes.err_apply_desc'),
        );
        return;
      }

      toast.success(
        t('recipes.success_applied'),
        t('recipes.success_applied_desc', {
          name: template.name,
          mediaName: selectedMedia?.name || t('recipes.selected_clip_default'),
        }),
      );
    } finally {
  ...
  if (!selectedClip || !selectedTargetType) {
    return (
      <div className="h-full flex-1 min-h-0 flex flex-col items-center justify-center p-6 text-center space-y-3">
        ...
        <div>
          <p className="text-sm font-semibold text-text-primary">{t('recipes.select_clip_first')}</p>
          <p className="mt-1.5 text-xs text-text-muted max-w-[240px] leading-relaxed mx-auto">
            {t('recipes.select_clip_first_desc')}
          </p>
        </div>
      </div>
    );
      ...
      <div className="p-4 border-b border-border bg-background-secondary/80 backdrop-blur sticky top-0 z-10 space-y-3">
        <div className="flex items-center gap-3 bg-background-tertiary rounded-xl p-2 pr-3 border border-border">
          <div className="w-10 h-10 rounded-lg bg-background-elevated flex items-center justify-center border border-border shrink-0">
            {selectedTargetType === 'video' ? <span className="text-primary/70 text-[10px]">{t('recipes.video_tag')}</span> : <span className="text-primary/70 text-[10px]">{t('recipes.image_tag')}</span>}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-text-primary truncate" title={selectedMedia?.name || selectedClip.id}>
              {selectedMedia?.name || t('recipes.selected_clip')}
            </p>
            <p className="text-[10px] text-text-muted mt-0.5">
              {selectedClip.duration.toFixed(1)}s • {t('recipes.recipes_applied', { count: appliedTemplates.length })}
            </p>
          </div>
        </div>
      ...
      <div className="px-4 py-3 border-b border-border/50 bg-background-secondary">
        <div className="flex flex-wrap gap-1.5">
          ...
          {EDITING_TEMPLATE_CATEGORIES.map((category) => (
            ...
            >
              {t('recipes.categories.' + category.id, category.name)}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 p-4 space-y-3">
        {filteredTemplates.length === 0 ? (
          <div className="py-12 text-center">
            <p className="text-text-secondary text-sm font-medium">{t('recipes.no_recipes_match')}</p>
            <p className="mt-2 text-xs text-text-muted">{t('recipes.try_different_search')}</p>
          </div>
        ) : (
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        ...
                        <div className="flex items-center justify-between mt-2.5">
                          <span className="text-[9px] uppercase tracking-wider text-text-muted font-medium">
                            {t('recipes.categories.' + template.category, formatCategoryLabel(template.category))}
                          </span>
                          <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            >
                              {applyingTemplateId === template.id ? t('recipes.btn_applying') : t('recipes.btn_apply')}
                            </button>
                          </div>
                        </div>
                      </div>
